...
- 方法内部按参数校验、状态切换、异步操作、失败回滚和结果通知分段，逻辑段之间保留空行。
- 注释解释并发版本、协议握手、坐标换算和容错清理等设计原因，不重复描述代码表面行为。
- guard 条件与后续主流程分开；连续的异步资源清理不能挤在同一个无间隔代码块中。
- 公开 API 命名与行为继续严格对齐 iOS，内部排版遵循本节规则。
